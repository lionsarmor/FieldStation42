...
def has_local_compiled_files() -> bool:
    return bool(catalog_files() or schedule_files())


def upload_compiled(config: dict | None = None) -> dict:
    config = config or load_config()
    if not catalog_files():
        raise CompiledSyncError("No compiled catalog files were found. Run env/bin/python station_42.py --rebuild_catalog first.")
    if not schedule_files():
        raise CompiledSyncError("No compiled schedule files were found. Run env/bin/python station_42.py --add_day first.")
